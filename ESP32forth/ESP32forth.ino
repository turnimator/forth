...
#define PLATFORM_OPCODE_LIST \
  ...
  /* Serial */ \
  X("Serial1.begin", SERIAL1_BEGIN, Serial1.begin(tos, SERIAL_8N1, 16, 17); DROPn(3)) \
  X("Serial1.end", SERIAL1_END, Serial1.end()) \
  X("Serial1.available", SERIAL1_AVAILABLE, PUSH Serial1.available()) \
  X("Serial1.read", SERIAL1_READ, PUSH Serial1.read()) \
  X("Serial1.readBytes", SERIAL1_READ_BYTES, n0 = Serial1.readBytes(b1, n0); NIP) \
  X("Serial1.write", SERIAL1_WRITE, n0 = Serial1.write(b1, n0); NIP) \
  ...
  Y(cmpx, PUSH compass.getX()) \
  Y(cmpy, PUSH compass.getY()) \
  Y(cmpz, PUSH compass.getZ()) \
  Y(cmpr, compass.read(); PUSH compass.getX(); PUSH compass.getY(); PUSH compass.getZ())
...
static Adafruit_SSD1305 *oled_display = new Adafruit_SSD1305(128, 64, &Wire, -1);
# define OPTIONAL_OLED_SUPPORT \
