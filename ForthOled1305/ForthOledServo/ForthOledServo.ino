...
#ifndef ENABLE_PCA9685_SUPPORT
# define OPTIONAL_PCA9685_SUPPORT
#else
...
void setServo(int num, int a){
  ...
  ang[num] = a;
  if (pwm > 150 && pwm < 590) {
    pwmController.setPWM(num, 0, pwm);
  }
}

int getServo(int num){
  int pwm = pwmController.getPWM(num); // Doesn't seem to work as I expected
  return ang[num];
}

#define OPTIONAL_PCA9685_SUPPORT\
  X("servo>", SET_PWM_CHANNEL, setServo(n1, n0); DROPn(2) ) \
  X("<servo", GET_PWM_CHANNEL, n0 = getServo(n0) ) \
